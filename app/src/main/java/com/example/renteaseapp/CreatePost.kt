...
class CreatePost : Fragment() {
    ...
    override fun onCreateView(
        ...
        region = view.findViewById(R.id.Regiontb)
        val regions = resources.getStringArray(R.array.region)
        val arrayAdapter = ArrayAdapter(requireContext(), R.layout.dropdownitem, regions)
        region.setAdapter(arrayAdapter)
        productName = view.findViewById(R.id.productet)
        productPrice = view.findViewById(R.id.priceet)
        productDescription = view.findViewById(R.id.descriptionet)
        imageName = view.findViewById(R.id.productImagetxt)
        btnRemImage = view.findViewById(R.id.removeProductImage)
        btnPickImage = view.findViewById(R.id.btnaddprofilephoto)
        btnPost = view.findViewById(R.id.ownerNewsfeedPostbtn)
    ...
    private fun checkAllFieldsFilled() {
        val allFieldsFilled = productName.text.isNotEmpty() &&
                region.text.isNotEmpty() && isImageUploaded && productPrice.text.isNotEmpty()
    ...
    private fun uploadImageToImgur() {
        ...
        try {
            imgurService.uploadImage(authHeader, imageBody).enqueue(object : Callback<ImgurResponse> {
    ...
    private fun uploadPostToFirestore() {
        try {
            ownerPosts.itemName = productName.text.toString()
            ownerPosts.price = productPrice.text.toString().toIntOrNull() ?: 0
            ownerPosts.description = productDescription.text.toString()
            ownerPosts.location = region.text.toString()
            ownerPosts.date = Timestamp.now()
            ownerPosts.type = "owner"
    ...
    private fun fetchUserDetailsAndStoreToSingleton(onComplete: () -> Unit = {}) {
        ...
        if (currentUser != null) {
            ...
            db.collection("users").document(uid).get()
                .addOnSuccessListener { document ->
                    if (document != null && document.exists()) {
                        ownerPosts.username = document.getString("username")
                        ownerPosts.profilePictureUrl = document.getString("profilePicture")
                        ownerPosts.uid = uid
    ...
    private fun loadUserProfileAndUsername() {
        try {
            ...
            val profilePictureUrl = ownerPosts.profilePictureUrl
            ...
            // Set the username
            val username = ownerPosts.username
